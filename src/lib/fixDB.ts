...
export default async (knex: Knex): Promise<void> => {
  ...
  const advancedAgentList = [
    { key: "scriptAgent", name: "剧本Agent", desc: "用于读取原文生成故事骨架、改编策略，建议使用具备强大文本理解和生成能力的模型" },
    { key: "productionAgent", name: "生产Agent", desc: "对工作流进行调度和管理，建议使用具备较强的逻辑推理和任务管理能力的模型" },
    { key: "universalAi", name: "通用AI", desc: "用于小说事件提取、资产提示词生成、台词提取等边缘功能，建议使用具备较强文本处理能力的模型" },
    { key: "ttsDubbing", name: "TTS配音", desc: "用于配音生成" },
  ...
  for (const agent of advancedAgentList) {
    // agent 配置全局共享：检查 NULL 全局行是否已有，没有则补一条
    const exists = await db("o_agentDeploy").where("key", agent.key).whereNull("userId").select("*").first();
    if (!exists) {
  ...
  //迁移供应商函数：vendor 配置全局共享，只看 NULL 全局行
  const data = await knex("o_vendorConfig").whereNull("userId").select("*");
